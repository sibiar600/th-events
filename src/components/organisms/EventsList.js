            ...
            <div className='h-auto flex justify-center bg-gray-100'>
                <div className='md:w-3/4 sm:w-full'>
                    ...
                    {isLoading ? <h1 className='text-2xl text-center'>Loading...</h1> : <div className='mt-6 grid md:grid-cols-2 xl:grid-cols-3 gap-2 relative'>
                        {events.map(event => (
                            <EventCard key={event.id} name={event.name} date={event.date} time={event.startHour} price={event.priceInYen} city={event.city} venue={event.venue} description={event.description} image={event.imageURL} />
                        ))}
                    </div>}
                </div>
            </div >
